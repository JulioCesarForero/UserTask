...
namespace ASPRad.Controller{
		...
		public async Task<IActionResult> Index(string? fieldname, string? fieldvalue, string? search, string? orderby, string ordertype = "desc", int page = 1, int limit = 10){
			try{
				...
				if(orderby != null){
					...
					query = query.OrderByField("user_id", false);
				}
				if(fieldvalue != null)
				{
	...
	/// <summary>
	/// Import csv file data into Users
	/// </summary>
	...
	public ActionResult ImportData([FromForm(Name = "file")] List<IFormFile> CsvFiles)
	{
		...
		public ActionResult Add([FromBody] UsersAddDTO postdata){
			try{
				...
				DB.SaveChanges();
				var recId = record.user_id; //newly created record id
				return Ok(record);
			}
			catch (Exception ex){
				return ServerError(ex);
		...
		public ActionResult Delete(string id){
			try{
				...
				query = query.Where(p => arrId.Contains(p.user_id.ToString()));
		...
		private async Task<IActionResult> ExportListRecords(List<Users> records)
		{
			...
			try{
				...
				if (exportFormat == "excel" || exportFormat == "csv")
				{
					...
					{
					    new DataHeader { Header = "User Id", Key = "user_id" },
					    new DataHeader { Header = "Title", Key = "title" },
					    new DataHeader { Header = "First Name", Key = "first_name" },
		...
		/// </summary>
   		/// <returns>PDF | CSV | Excel | HTML File</returns>
		[HttpGet]
		private async Task<IActionResult> ExportViewRecords(List<Users> records)
		{
			string exportFormat = Request.Query["export"].ToString().ToLower();
