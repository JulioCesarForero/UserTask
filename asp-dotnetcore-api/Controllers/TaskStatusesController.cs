...
namespace ASPRad.Controller{
	...
	/// <summary>
	/// Controller for Taskstatuses api
	/// </summary>
	public class TaskstatusesController:BaseController
	{
		...
		public TaskstatusesController(AppDBContext dbContext, IMapper mapper, IHttpContextAccessor httpContextAccessor, IWebHostEnvironment environment, IConfiguration Configuration, EmailHelper mailer, Rbac _rbac) :base(dbContext, httpContextAccessor, environment, Configuration)
		{
		...
		/// <param name="fieldvalue">Filter value.</param>
   		/// <returns>JSON Array of TaskStatuses records</returns>
		[HttpGet]
		public async Task<IActionResult> Index(string? fieldname, string? fieldvalue, string? search, string? orderby, string ordertype = "desc", int page = 1, int limit = 10){
			try{
				var query = from TaskStatuses in DB.TaskStatuses
							select new TaskStatuses {
								task_status_id = TaskStatuses.task_status_id,
						status_name = TaskStatuses.status_name
							};
				if(search != null){
					query = query.Where(
						p => EF.Functions.Like(p.task_status_id.ToString(), $"%{search}%") || 
						EF.Functions.Like(p.status_name, $"%{search}%") 
					);
				}
				if(orderby != null){
					...
					query = query.OrderByField("task_status_id", false);
				}
				if(fieldvalue != null)
				{
	...
	/// <summary>
	/// Import csv file data into TaskStatuses
	/// </summary>
	...
	public ActionResult ImportData([FromForm(Name = "file")] List<IFormFile> CsvFiles)
	{
			...
			foreach (IFormFile file in CsvFiles)
			{
				if (file.Length > 0) // ensures file has readable content
				{
					...
					using (var memoryStream = new MemoryStream())
					{
						...
						var records = csvReader.GetRecords<TaskStatuses>().ToList();

						DB.TaskStatuses.AddRange(records);
						DB.SaveChanges();
		...
		/// <summary>
		/// Select single TaskStatuses record by ID
		/// </summary>
		/// <param name="id">Table primary key.</param>
   		/// <returns>JSON Object of TaskStatuses record</returns>
		[HttpGet]
		[Route("api/taskstatuses/view/{id}")]
		public async Task<IActionResult> Detail(int id){
			try{
				...
				query = query.Where(p => p.task_status_id.Equals(id));
				// export page records
				if (Request.Query.ContainsKey("export")){
					var exportRecords = query.ToList();
		...
		/// </summary>
   		/// <returns>JSON Object of newly created TaskStatuses record</returns>
		[HttpPost]
		public ActionResult Add([FromBody] TaskstatusesAddDTO postdata){
			try{
				if (!ModelState.IsValid)
				{
					return BadRequest(ModelState);
				}
				var modeldata = new TaskStatuses();
								modeldata.status_name = postdata.status_name;
				// save TaskStatuses record
				DB.TaskStatuses.Add(modeldata);
				var record = modeldata; //newly created record
				DB.SaveChanges();
				var recId = record.task_status_id; //newly created record id
				return Ok(record);
			}
			catch (Exception ex){
				return ServerError(ex);
		...
		/// <summary>
		/// Get  TaskStatuses record for edit
		/// </summary>
		...
		/// <summary>
		/// Update  TaskStatuses record with form data
		/// </summary>
		/// <param name="id">Select record by table primary key.</param>
   		/// <returns>JSON Object of TaskStatuses record</returns>
		[HttpPost]
		public ActionResult Edit(int id, [FromBody] TaskstatusesEditDTO postdata){
			try{
				if (!ModelState.IsValid)
				{
					return BadRequest(ModelState);
		...
		public ActionResult Delete(string id){
			try{
				...
				query = query.Where(p => arrId.Contains(p.task_status_id.ToString()));
				DB.TaskStatuses.RemoveRange(query);
				DB.SaveChanges();
				return Ok(id);
			}
			catch (Exception ex){
		...
		/// <summary>
		/// Export  TaskStatuses records to different format such as: pdf, cvs, excel, html
		/// </summary>
   		/// <returns>PDF | CSV | Excel | HTML File</returns>
		[HttpGet]
		private async Task<IActionResult> ExportListRecords(List<TaskStatuses> records)
		{
			string exportFormat = Request.Query["export"].ToString().ToLower();
			string viewName = "ExportList"; // html template for pdf

			try{
				var fileName = "TaskStatusesReport";
				if (exportFormat == "excel" || exportFormat == "csv")
				{
					List<DataHeader> Columns = new List<DataHeader>()
					{
					    ...
					    new DataHeader { Header = "Status Name", Key = "status_name" }
					};
					var dataTable = records.ToDataTable(Columns);
					dataTable.TableName = "TaskStatuses"; // Excel worksheet title
		...
		/// </summary>
   		/// <returns>PDF | CSV | Excel | HTML File</returns>
		[HttpGet]
		private async Task<IActionResult> ExportViewRecords(List<TaskStatuses> records)
		{
			string exportFormat = Request.Query["export"].ToString().ToLower();
