...
import TaskstatusesAddPage from 'pages/taskstatuses/Add';
...
const defaultProps = {
	...
	apiPath: 'taskstatuses/index',
	routeName: 'taskstatuseslist',
...
const TaskstatusesListPage = (componentProps) => {
	...
	function ActionButton(data){
		...
		{
			label: "View",
			command: (event) => { app.navigate(`/taskstatuses/view/${data.task_status_id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('taskstatuses/view')
		},
		{
			label: "Edit",
			command: (event) => { app.navigate(`/taskstatuses/edit/${data.task_status_id}`) },
			icon: "pi pi-pencil",
	...
	function TaskStatusIdTemplate(data){
		if(data){
			return (
				<Link to={`/taskstatuses/view/${data.task_status_id}`}> { data.task_status_id }</Link>
	...
	function ImportData() {
		if (props.importData) {
			return (
				<ImportPageData label="Select a file to import" uploadPath="taskstatuses/importdata" buttonIcon="pi pi-folder" buttonLabel="Import Data" onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
			);
		}
	}
	...
	<CanView pagePath="taskstatuses/delete">
		<MultiDelete />
	</CanView>

				<ExportData />
	<CanView pagePath="taskstatuses/importdata">
		<ImportData />
	</CanView>
	...
	return (
<main id="TaskstatusesListPage" className="main-page">
    { (props.showHeader) && 
        ...
        <div className="container-fluid">
            <div className="flex flex-wrap gap-4 items-center gap-4">
                ...
                <div className="col-span-full " >
                    <Title title="Task Statuses"   titleClass="text-2xl text-primary font-bold" subTitleClass="text-gray-500"      separator={false} />
                </div>
            </div>
        </div>
        <hr />
    ...
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="flex flex-wrap justify-between items-center gap-3">
                <div className="col-span-full " >
                    <CanView pagePath="taskstatuses/add">
                        <Button label="Add Task Status" icon="pi pi-plus"  onClick={()=>app.openPageDialog(<TaskstatusesAddPage isSubPage apiPath={`/taskstatuses/add`} />, { closeBtn: true  })}  className="p-button w-full bg-primary "  />
                    </CanView>
                </div>
                <div className="col-span-full " >
                    <IconField>
                    ...
                    <div >
                        ...
                        <div className="page-records">
                            <DataTable 
                                ...
                                <Column  field="status_name" header="Status Name"   ></Column>
...
export default TaskstatusesListPage;
